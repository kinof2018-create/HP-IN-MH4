...
encoding.html   ← 인코딩 관리(김남주바이오) (독립 실행 가능)   · 발주/코드 데이터 내장
report.html     ← 경영자 보고서 이력 관리   (독립 실행 가능)   · GitHub REPORT/ 폴더 연동
...
## 탭 잠금(비밀번호) · 관리자
- 우측 상단 **🔒 잠금 설정** 버튼 → 관리자 4자리 비밀번호로 진입(최초 1회 설정).
- 관리자 패널에서 6개 탭마다 4자리 비밀번호를 **설정·변경·삭제**할 수 있습니다.
- 비밀번호가 설정된 탭은 클릭 시 입력창이 뜨고, 맞아야 진입됩니다(세션 동안 유지, 새로고침 시 다시 잠김).
- 정적 사이트라 설정은 **관리자 PC 브라우저(localStorage)** 에 저장됩니다. 모든 직원 PC에 동일 적용하려면
  관리자 패널 하단의 설정을 복사해 `index.html` 의 `var LOCK_DEFAULT = ...` 줄을 교체 후 재업로드하세요.
- 참고: 잠금은 통합 화면(index.html) 진입을 막는 **접근 편의 잠금**이며, 개별 파일(hppk.html 등) 직접 주소 접근까지
  막는 강력한 보안은 아닙니다(그 경우 각 탭 파일 수정 필요).
